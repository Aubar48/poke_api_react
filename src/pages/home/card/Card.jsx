  ...
  return (
    <div className={css.card}>
      ...
      >
        ...
        <h4 className={css.habitat_card}>
          habitat: {especiePokemon?.data?.habitat?.name}{" "}
        </h4>
        <div className={css.stats_card}>
          {itemPokemon?.stats?.map((sta, index) => {
            return (
              <h6 key={index} className={css.stats_item_card}>
                <span className={css.stats_name_card}> {sta.stat.name} </span>
                <progress value={sta.base_stat} max={110}></progress>
                <span className={css.stats_numero_card}> {sta.base_stat} </span>
              </h6>
            );
          })}
        </div>
        <div className={css.stats_type_card}>
          {itemPokemon?.types?.map((type, index) => {
            return (
              <h6
                className={`color-${type.type.name} ${css.stats_color_type_card}`}
                key={index}
              >
                {" "}
                {type.type.name}{" "}
